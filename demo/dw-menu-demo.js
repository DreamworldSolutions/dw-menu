...
export class DwMenuDemo extends LitElement {
  ...
  render() {
    return html`
      <dw-menu id="popover" heading="Popover heading" showClose placement="bottom-end" .actions=${actions} .disabledActions=${disabledActions} .hiddenActions=${hiddenActions} @action=${(e) => console.log("demo", e.detail)}></dw-menu>
      <label>Popover</label>
      <dw-icon-button icon="more_vert" @click=${this._onPopover}></dw-icon-button>

      <dw-menu id="bottom" heading="demo heading" showClose mobile-mode .actions=${actions} .disabledActions=${disabledActions} .hiddenActions=${hiddenActions} @action=${(e) => console.log("demo", e.detail)}></dw-menu>
